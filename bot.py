...
CHECKCARD_SHOP_ID  = os.environ.get("CHECKCARD_SHOP_ID")  or "123210"
CHECKCARD_SHOP_KEY = os.environ.get("CHECKCARD_SHOP_KEY") or "S7KPDILN7H"
CHECKCARD_BASE_URL = "https://checkcard.uz/api"

# ─── RAILWAY / WEBHOOK KONFIGURATSIYASI ─────────────────────
# RAILWAY_URL ni environment dan o'qiymiz — deploy qilganda to'g'ri URL bo'lishi kerak
RAILWAY_URL = os.environ.get("RAILWAY_URL") or "https://nexoratvbot-production.up.railway.app"
CHECKCARD_WEBHOOK_PATH = "/checkcard_webhook"
CHECKCARD_WEBHOOK_FULL_URL = f"{RAILWAY_URL.rstrip('/')}{CHECKCARD_WEBHOOK_PATH}"
...
# CheckCard webhook queue — thread-safe
import queue as _queue_module
_WEBHOOK_QUEUE: _queue_module.Queue = _queue_module.Queue()
_SEEN_MAX = 1000
...
async def cb_topup_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    action, pid = parts
    pay = RAM.pending_payments.get(pid)
    if not pay or pay.get("type") not in ("topup", "topup_manual"):
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("admin_state") == "topup_amount":
        ...
        # Webhook handler ham ishlatishi uchun pending_payments ga ham yozamiz
        if topup_msg_id:
    ...
    if is_any_admin(uid) and context.user_data.get("channel_manage_menu"):
        ...
        # Admin panel tugmalarini (maj_kanal va h.k.) ishlata olsin.
...
class _WebhookHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        try:
            ...
            # Thread-safe: to'lov ma'lumotini queue ga qo'yamiz
            # Asosiy event loop uni qayta ishlaydi
...
async def _handle_checkcard_webhook(data: dict):
    ...
    try:
        ...
        if _BOT_APP and _BOT_APP.bot:
            # To'lov xabarini (karta + Bekor qilish tugmasi) o'chiramiz
            wh_topup_msg_id = found_pay.get("topup_msg_id")
            if wh_topup_msg_id:
                try:
                    await _BOT_APP.bot.delete_message(chat_id=int(uid_str), message_id=wh_topup_msg_id)
                except Exception:
                    pass
            try:
...
def main():
    ...
    async def _startup_notify(context_job):
        try:
            ...
            webhook_url = CHECKCARD_WEBHOOK_FULL_URL
            await context_job.bot.send_message(
                ADMIN_ID,
                f"🚀 <b>Bot v23 Railway da ishga tushdi!</b>\n\n"
                f"💾 RAM: <b>{len(RAM.movies)}</b> kino, <b>{len(RAM.users)}</b> user\n"
                f"📦 Storage: {storage_msg}\n\n"
                f"💳 CheckCard Shop ID: <code>{CHECKCARD_SHOP_ID}</code>\n🔑 Shop Key: <code>{CHECKCARD_SHOP_KEY}</code>\n\n🔗 Webhook URL:\n<code>{webhook_url}</code>\n"
    ...
    async def _process_webhook_queue(context):
        """Webhook queue dan kelgan to'lovlarni qayta ishlaydi (har 5 soniyada)."""
        processed = 0
        while not _WEBHOOK_QUEUE.empty() and processed < 10:
            try:
                wh_data = _WEBHOOK_QUEUE.get_nowait()
                await _handle_checkcard_webhook(wh_data)
                processed += 1
            except Exception as e:
                logger.error(f"Webhook queue process xato: {e}")
                break

    if app.job_queue:
        app.job_queue.run_once(_startup_notify, when=5)
        app.job_queue.run_repeating(_periodic_sync, interval=120, first=60)
        app.job_queue.run_repeating(_process_webhook_queue, interval=5, first=5)
        logger.info("🔄 Periodik sync + webhook queue yoqildi")

    logger.info(f"🚀 Bot v23 Railway ishga tushdi! RAM: {len(RAM.movies)} kino, {len(RAM.users)} user")
